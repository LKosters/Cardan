<template>
    <nav ref="navRoot" class="bg-secondary-alt w-max fixed bottom-14 left-0 right-0 mx-auto rounded-[10px] px-[35px] py-[7px]" role="navigation" aria-label="Main navigation"
        @focusin="onNavFocusIn" @focusout="onNavFocusOut"
    >
        <ul class="flex items-center gap-2">
            <li v-for="(item, index) in navItems" :key="index" class="hover:bg-white rounded-b-[10px] duration-300 cursor-pointer" :aria-haspopup="item.items ? 'true' : undefined" :aria-expanded="activeIndex === index ? 'true' : 'false'" :tabindex="0" :aria-label="item.items ? 'Open menu' : 'Navigation item'" role="menuitem"
            ...
            >
                <transition name="fade" mode="out-in">
                    <div v-if="item.items && activeIndex === index" class="transition-opacity duration-300" >
                        <UiNavPopup :items="item.items" :autoFocus="focusedNavIndex === index" :key="focusedNavIndex === index ? 'focus' + index : 'nofocus' + index" @focusin="onNavFocusIn" @focusout="onNavFocusOut" />
                    </div>
                </transition>
